...
// Copied from MockCCIPRouter but with a few changes to enable asynchronous behaviour (via inheritance)
...
contract MockCCIPRouter is IRouter, IRouterClient {
  ...
  function routeMessage(
    ...
    return _routeMessageNow(message, gasForCallExactCheck, gasLimit, receiver);
  }
  ...
  function _routeMessage(
    ...
    uint256 gasLimit,
    address receiver
  ) internal virtual returns (bool success, bytes memory retData, uint256 gasUsed) {
    return _routeMessageNow(message, gasForCallExactCheck, gasLimit, receiver);
  }

  function _routeMessageNow(
    Client.Any2EVMMessage memory message,
    uint16 gasForCallExactCheck,
    uint256 gasLimit,
    address receiver
  ...
  function _fromBytes(bytes calldata extraArgs) internal pure returns (Client.GenericExtraArgsV2 memory) {
